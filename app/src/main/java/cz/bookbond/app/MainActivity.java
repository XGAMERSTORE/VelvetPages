...
public class MainActivity extends Activity {
    ...
    private void addDiscoverCard(LinearLayout parent, JSONObject u){
        ...
        TextView av=tv(u.optString("avatar","📚"),32,WINE,true);av.setGravity(Gravity.CENTER);av.setBackground(bg(Color.rgb(249,236,232),28,Color.TRANSPARENT,0)); head.addView(av,new LinearLayout.LayoutParams(dp(62),dp(62)));
    ...
    private void addPostCard(LinearLayout b,JSONObject p){LinearLayout c=card();JSONObject ex=p.optJSONObject("expand");JSONObject au=ex==null?null:ex.optJSONObject("author");String name=au==null?"Čtenář":au.optString("displayName","Čtenář");String avatar=au==null?"📚":au.optString("avatar","📚");String kind=p.optString("kind","status");String icon=kind.equals("recommendation")?"★":kind.equals("trail")?"⌁":"✦";TextView head=tv(avatar+"  "+name+"   "+icon,15,WINE,true);c.addView(head);gap(c,10);String book=p.optString("book","");if(!book.isEmpty()){TextView bk=tv("📖 "+book,13,GOLD,true);c.addView(bk);gap(c,8);}c.addView(tv(p.optString("text",""),16,INK,false));b.addView(c);}
